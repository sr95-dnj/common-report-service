...
<div style="font-size: 24px; ">
    ...
    <table width="100%">
        ...
        <tr>
            ...
            <td style="width: 3%">:</td>
            <td style="width: 20%"><span>{{$Controller::entoBn($data->master->presentDailyOrder->orderNumber,'number')}}</span></td>
            <td style="width: 5%"></td>
        ...
        <tr>
            ...
            <td style="width: 3%">:</td>
            <td style="width: 20%">
			 <span>{{$Controller::entoBn( optional($data->master->previousDailyOrder)->orderNumber,'number')}}</span>
					</td>
            <td style="width: 5%"></td>
            <td style="width: 10%">তারিখ</td>
            <td style="width: 3%;">:</td>
    ...
    <table class="center tbBorder" style="text-align: center; padding: 5px;">
            <thead>
                  <tr>
                        ...
                        <th style="width: 20%; padding: 2px; ">ব্যক্তিগত নম্বর, শাখা</th>
                        <th style="width: 25%; padding: 2px; ">পদবি ও নাম</th>
						<th style="width: 15%; padding: 2px; ">হইতে</th>
						<th style="width: 15%; padding: 2px; ">পর্যন্ত</th>
						<th style="width: 10%; padding: 2px; ">মোট দিন</th>
						<th style="width: 10%; padding: 2px; ">ছুটির প্রকৃতি</th>
                    </tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: left; width: 20%; padding: 2px;">
						    <span>{{$Controller::entoBn(optional($list->employee)->code,'number')}}/
							{{ optional($list->employee->employeeOfficialInformation->branch)->banglaName }}</span>
						...
						@foreach ($list->leaveApplicationList as $index => $leave)
						<span>{{$Controller::enToBnConveter($Controller::dateFormatter(optional($leave)->fromDate))}}
						</span> 
						...
						<td style="text-align: left; width: 15%; padding: 2px;">
						@foreach ($list->leaveApplicationList as $index => $leave)
						<span>{{$Controller::enToBnConveter($Controller::dateFormatter(optional($leave)->toDate))}} 
						</span>
						...
						@foreach ($list->leaveApplicationList as $index => $leave)
						<span>{{$Controller::entoBn(optional($leave)->totalDays,'number')}} দিন </span>
						 @endforeach
						</td>
						<td style="text-align: left; width: 10%; padding: 2px;">
						@foreach ($list->leaveApplicationList as $index => $leave)
						<span>{{optional($leave->leaveType)->banglaName}}
						</span>	 
						 @endforeach
						</td>
                    </tr>
