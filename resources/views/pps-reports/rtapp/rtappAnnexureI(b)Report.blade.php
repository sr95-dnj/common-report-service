...
$en = [
    ...
    'package_as_per_dpp_tpp_service' => 'Description of Procurement item as per DPP/TAPP SERVICES',
    ...
    'invitation_for_tender' => 'Issue of RFP',
    'invitation_for_prequal_work' => 'Invitation for EOI',
...
$bn = [
    ...
    'package_as_per_dpp_tpp_service'=> 'ডিপিপি/টিপিপি অনুযায়ী ক্রয়ের জন্য প্যাকেজের বর্ননা সেবা',
//        'package_as_per_dpp_tpp_work'=> 'ডিপিপি/টিপিপি অনুযায়ী ক্রয়ের জন্য প্যাকেজের বর্ননা\n' + 'পুর্ত কাজ',
    ...
    'service_procurement' => 'সেবার মোট ক্রয়মুল্যঃ',
    'bdt' => 'বাংলাদেশি টাকা',
    'project_cost' => 'Project Cost (Taka in Lac)',
    'issue_of_RFP' => 'Issue of RFP',
    'completion_of_contract_service' => 'Completion of Contract',
...
$rtappGoodsAndServiceList = json_decode($val['rtappGoodsAndServiceList']);
...
<table style="width: 100%; margin-bottom: 20px">
    <tr class="font-17 fontBold textRight">
        <td>Annexure - I (b)</td>
    </tr>
    <tr class="textRight text-green">
        <td style="font-size: 10px">Ref : PPR 2008</td>
    </tr>
    <tr class="fontBold textCenter">
...
<table class="tbBorder" style="width: 100%; margin-top: 20px">
    ...
    <tr class="textCenter fontBold font-13">
        ...
        <td rowspan="2">{{$L['package_as_per_dpp_tpp_service']}}</td>
    ...
    foreach ($rtappGoodsAndServiceList->list as $index=> $rtappGoodsAndServiceList){
        $totalEstdCostAmount = $totalEstdCostAmount + $rtappGoodsAndServiceList->estdCostAmount;
        ?>
        <tr class="textCenter font-13">
            <td>
                <?php
                $value = ($lng === 'bn')? $Controller::entoBn($rtappGoodsAndServiceList->packageName,'number') : $rtappGoodsAndServiceList->packageName;
                echo $value;
                ?>
            <td>{{$rtappGoodsAndServiceList->packageDppTppService}}</td>
            <td>
                @if($lng == 'en')
                    {{$rtappGoodsAndServiceList->unitType->unitTypeNameEng}}
                @else
                    {{$rtappGoodsAndServiceList->unitType->unitTypeNameBng}}
                @endif
            </td>
            <td>
                <?php
                $value = ($lng === 'bn')? $Controller::entoBn($rtappGoodsAndServiceList->quantity,'number') : $rtappGoodsAndServiceList->quantity;
                echo $value;
                ?>
            </td>
            <td>
                @if($lng == 'en')
                    {{$rtappGoodsAndServiceList->procurementMethod->nameEn}}-{{$rtappGoodsAndServiceList->procurementType->nameEn}}
                @else
                    {{$rtappGoodsAndServiceList->procurementMethod->nameBn}}-{{$rtappGoodsAndServiceList->procurementType->nameBn}}
                @endif
            </td>
            <td>{{$rtappGoodsAndServiceList->contractApproveAuthoriity}}</td>
            <td>{{$rtappGoodsAndServiceList->sourceOfFund}}</td>
            <td>
                <?php
                $value = ($lng === 'bn')? $Controller::entoBn($rtappGoodsAndServiceList->estdCostAmount,'number') : $rtappGoodsAndServiceList->estdCostAmount;
                echo $value;
                ?>
            </td>
            <td>
                ...
                if($lng == 'en'){
                    echo date('d-m-Y', strtotime($rtappGoodsAndServiceList->invitationForEoi));
                }
                else{
                    $d = explode("-",$rtappGoodsAndServiceList->invitationForEoi);
            ...
            <td>
                <?php
                $d = explode("-",$rtappGoodsAndServiceList->issueOfRfp);
                $d2 = explode("T", $d[2]);
                if($lng === 'en'){
                    echo $d2[0]. '-';
                    echo $d[1]. '-';
            ...
            <td>
                ...
                //echo date('d-m-Y', strtotime($val['advertisementEndDate']));
                if($lng == 'en'){
                    echo date('d-m-Y', strtotime($rtappGoodsAndServiceList->signingOfContract));
                }
                else{
                    $d = explode("-",$rtappGoodsAndServiceList->signingOfContract);
                    echo '<span style="font-size:18px !important">';
                    echo $Controller::entoBn($d[2],'number').'-';
                    echo $Controller::entoBn($d[1],'number').'-';
                    echo $Controller::entoBn($d[0],'number');
            ...
            <td>
                ...
                //echo date('d-m-Y', strtotime($val['advertisementEndDate']));
                if($lng == 'en'){
                    echo date('d-m-Y', strtotime($rtappGoodsAndServiceList->completionOfContract));
                }
                else{
                    $d = explode("-",$rtappGoodsAndServiceList->completionOfContract);
                    echo '<span style="font-size:18px !important">';
                    echo $Controller::entoBn($d[2],'number').'-';
                    echo $Controller::entoBn($d[1],'number').'-';
                    echo $Controller::entoBn($d[0],'number');
    ...
    <?php } ?>
    <tr class="textCenter fontBold font-13">
        <td colspan="2">{{$L['service_procurement']}}</td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
