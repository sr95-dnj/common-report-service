        ...
        @foreach ($data as $key=>$value)
            @foreach ($value as $key1=>$cValue)
                <tr>
                    ...
                    @if ($cValue->indicatorId==$cValue->drfChildIndicatorid)
                        <td rowspan="{{count($value)}}" colspan="{{(count($value)==1)? 2:0}}"
                            style="vertical-align: top;"> 
                            @if($lng == 'en')
                               {{$cValue->indicatorNameEn}} 
                            @else
                                {{$cValue->indicatorNameBn}} 
                            @endif                            
                        </td>
                    @endif
                    ...
                    @if (count($value)>1)
                        <td style="height: 53px !important;">
                            @if ($cValue->indicatorId !=$cValue->drfChildIndicatorid)
                                @if (isset($cValue->indicatorNameEn))
                                    @if($lng == 'en')
                                         {{$cValue->indicatorNameEn}}
                                    @else
                                         {{$cValue->indicatorNameBn }}
                    ...
                    @if ($cValue->indicatorId==$cValue->drfChildIndicatorid)
                        <td rowspan="{{count($value)}}" style="vertical-align: top;">
                            @if (isset($cValue->dataSourcesStr))
                                <?php echo $cValue->dataSourcesStr ?>
                            @endif
                        </td>
                    @endif
                    ...
                    @if ($cValue->indicatorId==$cValue->drfChildIndicatorid)
                        <td rowspan="{{count($value)}}" style="vertical-align: top;">
                            @if (isset($cValue->ministryDivisionsStr))
                                <?php echo $cValue->ministryDivisionsStr ?>
                            @endif
                        </td>
                    @endif

                    <td>
                        @if (isset($cValue->baseLineEntry))
                            {{$cValue->baseLineEntry}}
                        @endif
                    </td>
                    ...
                    @foreach ($fiscalYear as $index=>$year)                       
                            <td>
                            @if ($index==0)
                                <?php echo $cValue->year1 ?>
                            @endif 
                            @if ($index==1)
                                <?php echo $cValue->year2 ?>
                            @endif 
                            @if ($index==2)
                                <?php echo $cValue->year3 ?>
                            @endif 
                            @if ($index==3)
                                <?php echo $cValue->year4 ?>
                            @endif 
                            @if ($index==4)
                                <?php echo $cValue->year5 ?>
                            @endif 
                            </td>                       
                    @endforeach                    
